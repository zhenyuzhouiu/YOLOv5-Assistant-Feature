...
dst_path = "/media/zhenyuzhou/Data/finger_knuckle_2018/FingerKnukcleDatabase/Finger-knuckle/mask-seg/01/"
...
for i in image_name:
    ...
    if cls == "01":
        image = cv2.imread(os.path.join(src_path, i))
        image = cv2.rotate(image, cv2.ROTATE_90_CLOCKWISE)
        cv2.imwrite(subject_path + '/' + i, image, [int(cv2.IMWRITE_JPEG_QUALITY), 100])
        num += 1
