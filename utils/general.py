...
def compute_loss(p, targets, model, csl_label_flag=True):
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            # print(gi.numpy())
            ps = pi[
                b, a, gj, gi]  # prediction subset corresponding to targets  前向传播结果与target信息进行匹配 筛选对应的网格 得到对应网格的前向传播结果
            ...
            pwh = (ps[:, 2:4].sigmoid() * 2) ** 2 * anchors[
                i]  # 对前向传播结果wh进行回归  （预测的是当前featuremap尺度上的框wh尺度缩放量）-> 处理成featuremap尺度上框的真实wh
...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):  # 3种步长的feature map
        ...
        tbox.append(torch.cat((gxy - gij, gwh),
                              1))  # 每个预测层的box shape(3*M, 4)  其中xy:当前featuremap尺度上的真实gt xy与负责预测网格坐标的偏移量  wh：当前featuremap尺度上的真实gt wh
...
def plot_one_rotated_box(rbox, img, color=None, label=None, line_thickness=None, pi_format=True):
    ...
    color = color or [random.randint(0, 255) for _ in range(3)]
    # rbox = np.array(x)
    if pi_format:  # theta in the range of [-pi/2,pi/2)
        rbox[-1] = (rbox[-1] * 180 / np.pi) + 90  # [0,179]

    # rect=[(x_c,y_c),(w,h), theta] theta:flaot[0-179]  -> (-180,0)

    rect = longsideformat2cvminAreaRect(rbox[0], rbox[1], rbox[2], rbox[3], (rbox[4] - 180))
    ...
    if rotated == 90:
        if abs(rect[2]) == 90:
            ...
            if width < height:
                ...
                img_crop = cv2.warpPerspective(img_crop, M, (1000, 1000))
            else:
        ...
        if abs(rect[2]) == 90:
            ...
            if width < height:
                dst_pts = np.array([[height - 1, width - 1],
                                    ...
                                    [height - 1, 0]], dtype="float32")
                M = cv2.getPerspectiveTransform(src_pts, dst_pts)
                img_crop = cv2.warpPerspective(img_crop, M, (height, width))
            else:
                dst_pts = np.array([[width - 1, 0],
                                    ...
                                    [0, 0]], dtype="float32")
                M = cv2.getPerspectiveTransform(src_pts, dst_pts)
                img_crop = cv2.warpPerspective(img_crop, M, (width, height))
        else:
            src_pts = poly.astype("float32")
            width = int(rect[1][0])
            height = int(rect[1][1])
    ...
    if label:
        ...
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, 5, cv2.LINE_AA)  # filled
        # cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 4, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, 2, [225, 255, 255], thickness=5, lineType=cv2.LINE_AA)
