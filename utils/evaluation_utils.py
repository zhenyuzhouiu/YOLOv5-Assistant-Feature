        ...
        with open(fullname, 'r') as f_in:
            ...
            for splitline in splitlines:  # splitline:每行中的m个元素
                ...
                if (oriname not in nameboxdict):
                    ...
                    nameboxdict_classname[
                        oriname] = []  # 弄个元组,汇集原图目标信息 eg: 'P706':[[poly1, confidence1,'classname'], ..., ]
...
def rbox2center(rbox, classname, conf, img_name, out_path, pi_format=False):
    """
       将分割图片的目标信息填入原始图片.txt中
       @param robx: rbox:[tensor(x),tensor(y),tensor(l),tensor(s),tensor(θ)]
       @param classname: string
    ...
    # 目标所属图片名称_分割id 置信度 poly classname
    lines = str(rbox[0].cpu().float().numpy()) + ' ' + str(rbox[1].cpu().float().numpy()) + ' ' + str(rbox[4].cpu().float().numpy()) + ' ' + str(int(classname.item())) + ' ' + str(int(conf.item() * 100))
    # 移除之前的输出文件夹,并新建输出文件夹
    if not os.path.exists(out_path):
        os.makedirs(out_path)  # make new output folder
